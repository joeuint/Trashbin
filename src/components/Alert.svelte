<script lang="ts">
    type AlertStyling = 'success' | 'danger' | 'info' | 'burn'
    export let styling: AlertStyling
</script>
<!-- TODO: Add rest of styles -->
{#if styling === 'burn'}
    <div class="rounded-lg p-3 text-lg border-2 border-orange-800 text-orange-400 mb-3">
            <div class="flex">
                <svg class="mr-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M12 22c-4.97 0-9-2.582-9-7v-.088C3 12.794 4.338 11.1 6.375 10c1.949-1.052 3.101-2.99 2.813-5l-.563-3l2.086.795c3.757 1.43 6.886 3.912 8.914 7.066A8.495 8.495 0 0 1 21 14.464V15c0 1.562-.504 2.895-1.375 3.965"/><path d="M12 22c-1.657 0-3-1.433-3-3.2c0-1.4 1.016-2.521 1.91-3.548L12 14l1.09 1.252C13.984 16.28 15 17.4 15 18.8c0 1.767-1.343 3.2-3 3.2Z"/></g></svg>
                <div class="font-medium mr-1">Burned! </div>
                <div class="font-normal"><slot /></div>
            </div>
    </div>
{:else if styling === 'info'}
<div class="rounded-lg p-3 text-lg border-2 border-blue-800 text-blue-400 mb-3">
    <div class="flex items-center">
        <svg class="mr-2" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M11 17h2v-6h-2v6Zm1-8q.425 0 .713-.288T13 8q0-.425-.288-.713T12 7q-.425 0-.713.288T11 8q0 .425.288.713T12 9Zm0 13q-2.075 0-3.9-.788t-3.175-2.137q-1.35-1.35-2.137-3.175T2 12q0-2.075.788-3.9t2.137-3.175q1.35-1.35 3.175-2.137T12 2q2.075 0 3.9.788t3.175 2.137q1.35 1.35 2.138 3.175T22 12q0 2.075-.788 3.9t-2.137 3.175q-1.35 1.35-3.175 2.138T12 22Z"/></svg>
        <div class="font-medium mr-1">Info! </div>
        <div class="font-normal"><slot /></div>
    </div>
</div>
{/if}
